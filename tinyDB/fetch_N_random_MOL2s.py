...
def main():
    ...
    parser.add_argument('-d','--outputDirectory',type=str, default='./tempMOL2/', help='Output directory that will contain the random subsample of compounds.')
    ...
    for tranche in progress_bar:
        ...
        mol2_ligands.extend(extract_resorted_mol2_archives(compounds,tranche))
    
    # writing compounds to files in specified directory
    Path(outputDirectory).mkdir(parents=True, exist_ok=True)
    for mol2_ligand in tqdm(mol2_ligands, desc=f'Writing Files to {outputDirectory}...', bar_format='{desc}|{bar:60}|{n}/{total} [{elapsed}<{remaining}]'):
        zincID = re.search('ZINC[0-9]+',str(mol2_ligand)).group(0)
        with open(f'{outputDirectory}/{zincID}.mol2','wb') as outputFile:
            outputFile.write(mol2_ligand)

def read_shuffled_sample_compounds(input_file):
    return [compound.strip().split(',') for compound in open(input_file)]


def extract_resorted_mol2_archives(compounds,tranche):
    indices = numpy.where(compounds == tranche)[0]
    tranche_zincIDs = numpy.char.add(compounds[indices][:,1].astype(str), '.mol2')
    tranche_zip = zipfile.ZipFile(f'./tinyMOL2_filtered/{tranche}.zip', 'r')
    mol2_fileslist = tranche_zip.namelist()
    mol2_files = [tranche_zip.read(f'{zincID}') for zincID in sort_sublist_by_original_list(mol2_fileslist, tranche_zincIDs)]
    return mol2_files
